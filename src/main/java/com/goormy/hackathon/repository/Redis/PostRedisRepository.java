...
public class PostRedisRepository {
    ...
    public PostRedis_DS getById(Long postId) {
        return valueOperations.get(POST_KEY + postId);
    }

    public void set(PostCache_SY postCacheSY) {
//        valueOperations1.set(postCacheSY.getKey(), postCacheSY);
        redisTemplate.opsForValue().set("post:" + postCacheSY.getId(), postCacheSY);
//        redisTemplate.opsForValue().set(postCacheSY.getKey(), postCacheSY);
    }
}
