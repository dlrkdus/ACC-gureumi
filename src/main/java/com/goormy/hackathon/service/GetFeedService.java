...
import com.goormy.hackathon.dto.response.GetFeedResponseDto_SJ;
...
public class GetFeedService {
    ...
    private List<PostSimpleInfo> getPushPullFeed(UserRedis userRedisDS) {
        // 날짜 세팅
        ...
        // feedUser 접근
        List<PostSimpleInfo> feedUserCache = feedUserRedisRepository.getAll(userRedisDS.getId());
        // push 캐시 가져오고 비우기 & 3일 전 포스트까지만 가져오기
        List<PostSimpleInfo> postSimpleInfoDSList = new ArrayList<>(feedUserCache.stream()
    ...
    // 5. size개의 post를 구분
    private List<Long> splitPostSimpleInfoList(UserRedis userRedisDS,
    ...
    public List<GetFeedResponseDto> getFeedList(Long userId, int size) {
        ...
        UserRedis userRedisDS = getUser(userId);

        // 2.0 이미 정리해둔 post가 있으면 가져오기
        List<Long> postIdList = feedUserSortRedisRepository.getSome(userId, size);
        if (postIdList.size() == size) {
            return getPostList(postIdList).stream().map((post) ->
                            GetFeedResponseDto.toDto(post, makePhotoUrl(post)))
                    .toList();
        } else {
        ...
        List<PostSimpleInfo> pushPullFeedList = getPushPullFeed(userRedisDS);
    ...
    // post의 url 만들기
    private String makePhotoUrl(PostRedis_DS post) {
        return photoService.getCDNUrl(post.getId(), post.getImageUrl());
    }
}
